...
class Increment(Command):
    ...
    def incrementVersion(self, manifestPath: Path, pos: str) -> None:
    ...
    @override
    async def run(self):
        self.incrementVersion(MANIFEST_EXTENSION, self.position)
        for file_path in Path(MANIFEST_FOLDER).iterdir():
            if file_path.is_file():
                self.incrementVersion(file_path, self.position)

class Get(Command):
    @override
    async def run(self):
        print(getJson(MANIFEST_EXTENSION)["version"])
